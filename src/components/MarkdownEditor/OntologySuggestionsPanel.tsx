...
import { UseOntologySuggestionsResult, OntologySuggestion } from '@/hooks/markdown-editor/types/ontology-suggestions';
      ...
      {(isLoading || isPending) ? (
        ...
        <>
          <div className="flex flex-wrap gap-2 mb-4">
            {suggestions.terms
              .filter((term: OntologySuggestion) => !term.applied && !term.rejected)
              .map((term: OntologySuggestion) => (
                <div 
                  key={term.id} 
          ...
          {suggestions.terms.filter((term: OntologySuggestion) => !term.applied && !term.rejected && (term.score || 0) >= 70).length > 1 && (
            <Button
              variant="outline"
              size="sm"
              className="w-full"
          ...
          {suggestions.terms.filter((term: OntologySuggestion) => term.applied).length > 0 && (
            <div className="mt-3">
              <h4 className="text-xs font-medium text-muted-foreground mb-2">Applied Terms</h4>
              <div className="flex flex-wrap gap-2">
                {suggestions.terms
                  .filter((term: OntologySuggestion) => term.applied)
                  .map((term: OntologySuggestion) => (
