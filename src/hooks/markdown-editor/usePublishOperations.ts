    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.log('Content enrichment error (non-critical):', enrichmentError);
        }
        ...
        // Explicitly handle errors from the save draft operation
        console.error('Error while saving draft before publishing:', saveDraftError);
        handleError(
          saveDraftError,
          "Failed to save document before publishing",
          { level: "error", technical: false }
        );
        return { success: false, error: "Failed to save document before publishing" };
      }
    } catch (error) {
