...
serve(async (req) => {
  ...
  try {
    ...
    const openaiResponse = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are a tag generation assistant. Extract 5-10 relevant tags from the provided content. Return ONLY an array of tags, nothing else."
          },
          {
            role: "user",
            content
    ...
    if (!openaiResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to generate tags from OpenAI" }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
